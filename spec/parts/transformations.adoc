...
Next, for all values **v~0~** in **b~0~**, for all values **qv~1~** from 0 to (**orig_range(1).max** - **orig_range(1).min**) / 4, Color Bucket **b~2~(v~0~,qv~1~)** is encoded if **v~1~ = qv~1~ * 4 + orig_range(1).min** is in the set **b~1~(v~0~)**:
